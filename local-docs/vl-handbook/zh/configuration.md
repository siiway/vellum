...
| 字段          | 必填 | 说明                                                                                                          |
| ------------- | :--: | ------------------------------------------------------------------------------------------------------------- |
| `provider`    |  ✓   | `"workers-ai"`、`"openai-compatible"` 或 `"anthropic"`。与 `aiSummary` 相同。                                |
| `model`       |      | 模型 ID。默认值：Llama 3.3 70B Fast / gpt-4o-mini / Haiku 4.5。                                              |
| `baseUrl`     |      | OpenAI 兼容端点。`VELLUM_AI_BASE_URL` 优先。                                                                  |
...
凭据沿用 `aiSummary` / `aiChat` 使用的同一组环境变量
（`VELLUM_AI_API_KEY`、`VELLUM_AI_BASE_URL`）。新增加的是承载缓存
的 D1 数据库：
